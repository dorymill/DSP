...
void main (void)
{
    ...
    lowpass(InputSignal_f32_1kHz_15kHz, outputSignal, sincKernel, SINC, 10000, SAMPLE_RATE, SIG_LEN, KERNEL_LEN);
    ...
    // Write kernel to file
    {
        ...
        for(int iter = 0; iter < KERNEL_LEN; iter++) {
            ...
            fprintf(pKernel, "\n%f %f", timeAxisDft[iter], sincKernel[iter]);
        }

        fclose(pKernel);

        // Save the output signal
        pOutputSig = fopen("../Signals/dft_filtered.dat", "w");

        for(int iter = 0; iter < SIG_LEN - KERNEL_LEN; iter++) {
            timeAxisDft[iter] = iter * (1 / (double) SAMPLE_RATE);
    ...
    // Calculate and save the filter response
    {
        pFilterResponse = fopen("../Signals/sinc_response.dat", "w");
        pFilterPhase    = fopen("../Signals/sinc_phase.dat", "w");

        for(int iter = 0; iter < SIG_LEN2 -1; iter++) {
            frequencyAxisKernel[iter] = (double) (iter*((SAMPLE_RATE/2)/((SIG_LEN)/2)));
        }

        // Get the response
        dft(sincKernel, sincReX, sincImX, KERNEL_LEN);
        dft_mag(sincReX, sincImX, sincMag, KERNEL_LEN/2);
        dft_phase(sincReX, sincImX, sincMag, KERNEL_LEN/2);

        // Save the mag and phase frequency response
        for(int iter = 0; iter < KERNEL_LEN/2; iter++) {
            fprintf(pFilterResponse, "\n%f %f", frequencyAxisKernel[iter], sincMag[iter]);
            fprintf(pFilterPhase, "\n%f %f", frequencyAxisKernel[iter], sincPhase[iter]);
        }

        fclose(pFilterResponse);
    }
