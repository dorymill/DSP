             ...
             int kernel_length)
{
        ...
        case (SINC):
            {
                ...
                for(int n = 0; n < kernel_length; n++){
                    ...
                    if((n - kernel_length/2) != 0) {
                        kernel[n] = sin(2*PI*f_c*(n - (double) kernel_length/2)) / (n - (double) kernel_length/2);
                        // Window this sucker
                        kernel[n] = kernel[n] * (0.54 - 0.46*cos(2*PI*n/ (double) kernel_length));
                    }
                }
    ...
    for(int i = kernel_length ; i < sig_length; i++) {
        for (int j = 0; j < kernel_length; j++) {
            output[i+j] += signal[i]*kernel[j];
        }
    }
    //convolve(signal, output, kernel, sig_length, kernel_length);
